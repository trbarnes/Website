...
@route('/')
def hello():
	return template('./static/download')
	
@route('/resume')
def resume():
	return template('./static/resume')
...
def game():
	...
	return template('./static/newgame', random_num=random_num)

@route('/game', method='POST')
def do_game():
    guess = int(request.forms.get('guess'))
    if (guess == random_num):
        return "<h1>You guessed correctly!</h1>"	
    else:
        return template('./static/game', guess=guess)
		
		
#Values for pieces
pvalue = { 'P': 100, 'N': 300, 'B': 315, 'R': 500, 'Q': 900, 'K': 1000000 }
...
#@route('/chess') THIS NEEDS A LOT OF WORK, COULD NOT GET WORKING
def chess():
    ...
    while True:
        ...
        while move not in hist[-1].move_generation():
            ...
            move = str(request.forms.get('move'))
        ...
        hist.append(hist[-1].move(move))
	#return template('./static/newchess', move=move)

@route('/chess', method='POST')
def do_chess():
    move = str(request.forms.get('move'))
    return template('./static/chess', move=move)	
